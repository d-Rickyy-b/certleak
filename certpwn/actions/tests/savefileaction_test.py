...
from certpwn.actions import SaveFileAction
...
class TestSaveFileAction(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.action = SaveFileAction(path="/test")
        self.assertEqual(".txt", self.action.file_ending)
        self.assertEqual("${data}", self.action.template)

    @patch("builtins.open")
    @patch("certpwn.actions.savefileaction.TemplatingEngine")
    def test_perform_path_not_exists(self, te_mock, open_mock):
        ...
        # We need to make sure that the file's name is the cert_index
        self.update.cert_index = "thisIsTheRythmOfTheFile"

        self.action.perform(self.update, "", [])
    ...
    def test_perform_file_ending_empty(self, te_mock, open_mock):
        ...
        self.update.cert_index = Mock()
        self.update.cert_index.__repr__ = Mock(return_value=file_name)
        ...
        # We called str(update.cert_index) once
        self.update.cert_index.__repr__.assert_called_once()
    ...
    def test_perform_file_ending(self, te_mock, open_mock):
        ...
        # We need to make sure that the file's name is the cert_index
        self.update.cert_index = Mock()
        self.update.cert_index.__repr__ = Mock(return_value="123456")
